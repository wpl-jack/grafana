...
func (srv RulerSrv) updateAlertRulesInGroup(c *models.ReqContext, groupKey ngmodels.AlertRuleGroupKey, rules []*ngmodels.AlertRule) response.Response {
	...
	err := srv.xactManager.InTransaction(c.Req.Context(), func(tranCtx context.Context) error {
		...
		// groupChanges contains changes explicitly made by the user. If those changes pass validation and authorization,
		// we need to generate changes for all rules in all affected groups.
		// This is needed to make sure that there is no concurrent update of any affected group as well as re-index alert groups in the case of if there are alerts that are moved between groups
		finalChanges = calculateAutomaticChanges(groupChanges)

		logger.Debug("updating database with the authorized changes", "add", len(groupChanges.New), "update", len(groupChanges.New), "delete", len(groupChanges.Delete), "automatic_updates", len(finalChanges.Update)-len(groupChanges.Update))
