...
func TestStaleResults(t *testing.T) {
	...
	t.Run("should mark missing states as stale", func(t *testing.T) {
		...
		// Init
		processed := st.ProcessEvalResults(ctx, rule, initResults)
		...
		processed = st.ProcessEvalResults(ctx, rule, results)
		...
		for _, s := range processed {
			if s.CacheId == expectedStaleReturned {
				assert.Truef(t, s.Stale, "Returned stale state should have Stale set to true")
				...
				assert.Equal(t, "", s.LastEvaluationString)
				break
			}
		}
		currentStates = st.GetStatesForRuleUID(orgID, rule.UID)
